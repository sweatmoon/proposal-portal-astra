...
function findTableByHeaders(tables, headers) {
    for (const t of tables) {
        ...
        if (headers.every(h => firstRow.includes(h)))
            return t;
    }
    return null;
}
...
function extractAnchorSectionHtml(html, anchorName) {
...
export function parsePersonnelHtml(html) {
    ...
    function findOffset(cells, ...keywords) {
        for (let ci = 0; ci < cells.length; ci++) {
            if (keywords.some(kw => cells[ci].includes(kw)))
                return ci;
        }
        return 0;
    }
    ...
    for (let i = 0; i < t4.length; i++) {
        ...
        if (rowText.includes('성명') && rowText.includes('직위')) {
            ...
            if (mName)
                personnel.name = mName[1];
            const mPos = raw.match(/[（(]([^,）)]+)/);
            if (mPos)
                personnel.position = mPos[1].trim();
        ...
        if (rowText.includes('감리원증') || rowText.includes('감리원 번호')) {
            ...
            for (let ci = off; ci < cells.length; ci++) {
                ...
                if (!val || val === '-')
                    continue;
                if (lbl.includes('감리원증') || lbl.includes('감리원 번호'))
                    personnel.auditor_cert_no = val;
                if (lbl.includes('감리원 등급') || lbl === '감리등급')
                    personnel.auditor_grade = val;
                if (lbl.includes('기술 등급') || lbl === '기술등급')
        ...
        if (rowText.includes('최종학교')) {
            ...
            for (let ci = off + 1; ci < cells.length; ci++) {
                ...
                if (lbl.includes('전공'))
                    personnel.major = val;
                if (lbl.includes('학위') || lbl.includes('졸업'))
                    personnel.degree = val;
            }
            if (!personnel.major && vRow[off + 1])
                personnel.major = vRow[off + 1];
            if (!personnel.degree && vRow[off + 2])
                personnel.degree = vRow[off + 2];
        }
        ...
        if (lblCell.includes('주요 경력') && !lblCell.includes('자격') && !lblCell.includes('및')) {
            if (valCell)
                personnel.career_summary = valCell;
        }
        if (lblCell.includes('주요 경력 및 자격') || lblCell.includes('주요경력및자격')) {
            if (valCell)
                personnel.career_qualif = valCell;
        }
        ...
        if (lblCell2.includes('시스템 개발') || lblCell2.includes('프로젝트 실무')) {
            if (valCell2)
                personnel.career_project = valCell2;
        }
        ...
        if (lblCell3.includes('주요 이력') || lblCell3.includes('전문가용')) {
            if (valCell3)
                personnel.career_expert = valCell3;
        }
    }
    // ── 이메일 전체 재스캔 (보조) ──────────────────────────
    if (!personnel.email) {
    ...
    const eduTable = findTableByHeaders(tables, ['교육명']) ??
    ...
    const auditTable = findTableByHeaders(tables, ['사업명', '참여율']) ??
    ...
    if (t9.length > 0) {
        ...
        for (let ci = 0; ci < hdr.length; ci++) {
            const h = hdr[ci];
            if (h.includes('기간') || h.includes('연도'))
                cPeriod = ci;
            // '경력'이 포함되더라도 '근거'가 같이 있으면 "유사 경력의 근거" 열이므로 제외
            if (h.includes('경력') && !h.includes('근거'))
                cCareer = ci;
            if (h.includes('담당'))
                cDuty = ci;
            if (h.includes('근거') || h.includes('비고'))
                cBasis = ci;
        }
    ...
    if (t9b.length > 0) {
        ...
        for (let ci = 0; ci < hdr.length; ci++) {
            const h = hdr[ci];
            if (h.includes('연도') || h.includes('기간'))
                cYear = ci;
            if (h.includes('프로젝트') || h.includes('사업명'))
                cProject = ci;
            if (h.includes('주관') || h.includes('발주') || h.includes('기관'))
                cClient = ci;
            if (h.includes('분야'))
                cDomain = ci;
            if (h.includes('역할'))
                cRole = ci;
            if (h.includes('소속') || h.includes('회사'))
                cCompany = ci;
            if (h.includes('비고'))
                cRemarks = ci;
        }
    ...
    // 실제 확인: index 11
    const certTable = findTableByHeaders(tables, ['자격증 명', '발급처']) ??
    ...
    for (const key of ['auditor_grade', 'auditor_cert_no', 'phone']) {
