 ...
 */
function extractLayoutNamesFromPptx(buf) {
    const names = [];
    try {
        ...
        console.warn('[extractLayouts] 파싱 실패:', e.message);
    }
    return names;
}
// ═══════════════════════════════════════════════════════════════════
...
app.post('/seed', async (c) => {
    try {
      ...
      `, [s.code, s.name, s.number, s.sort]);
        ...
        // 섹션 ID 맵
        const secRows = await query(`SELECT id, menu_code FROM ppt_menus WHERE menu_code = ANY($1)`, [SECTIONS.map(s => s.code)]);
        const SEC = {};
        for (const r of secRows)
            SEC[r.menu_code] = r.id;
        const MENUS = [
            // ── 다. 감리 일정 및 절차 ─────────────────────────────────────
            {
      ...
      `, [parentId, m.code, m.name, m.number, m.sort, m.enabled]);
            menuInserted++;
            const menu = await queryOne(`SELECT id FROM ppt_menus WHERE menu_code=$1`, [m.code]);
            if (!menu)
                continue;
            // 생성규칙 upsert
            await exec(`
...
app.post('/restore', async (c) => {
    try {
        ...
        if (!id || !menu_code || !menu_name)
            return c.json({ ok: false, error: 'id, menu_code, menu_name 필수' }, 400);
        // 메뉴 upsert (원본 id 보존)
        await exec(`
    ...
    `, [id, parent_id ?? null, menu_code, menu_name, menu_number ?? null, sort_order ?? 0, is_enabled ?? 1]);
        // rule upsert (있을 때만)
        if (rule) {
            const { generation_mode, template_strategy, calculator_code, renderer_code, pagination_mode, postprocess_mode, merge_strategy, rule_config } = rule;
            await exec(`
        INSERT INTO ppt_generation_rules
          (menu_id, generation_mode, template_strategy, calculator_code, renderer_code,
           pagination_mode, postprocess_mode, merge_strategy, rule_config)
...
app.put('/:id/rule', async (c) => {
    try {
        const id = Number(c.req.param('id'));
        const body = await c.req.json();
        const { generation_mode, template_strategy, calculator_code, renderer_code, pagination_mode, postprocess_mode, merge_strategy, rule_config, target_layout_name, } = body;
        await exec(`
      INSERT INTO ppt_generation_rules
        (menu_id, generation_mode, template_strategy, calculator_code, renderer_code,
...
app.post('/:id/templates', async (c) => {
    try {
        ...
        let variant_code;
        let capacity;
        let pptx_b64_key = null;
        let pptx_file_path = null;
        ...
        if (!template_name)
            return c.json({ ok: false, error: '템플릿 이름은 필수입니다' }, 400);
        const row = await queryOne(`
      INSERT INTO ppt_templates
...
app.post('/compositions/reorder', async (c) => {
    try {
        ...
        for (const item of body) {
            await exec(`UPDATE ppt_compositions SET sort_order=$1, updated_at=NOW() WHERE id=$2`, [item.sort_order, item.id]);
        }
...
app.post('/presets', async (c) => {
    try {
        ...
        if (!name || !Array.isArray(snapshot))
            return c.json({ ok: false, error: 'name, snapshot 필수' }, 400);
        const row = await queryOne(`
