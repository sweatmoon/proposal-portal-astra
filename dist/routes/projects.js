...
app.delete('/:id', async (c) => {
    ...
    if (isNaN(id))
        return c.json({ ok: false, error: 'invalid id' }, 400);
    const existing = await queryOne('SELECT id, project_name FROM audit_projects WHERE id = $1', [id]);
    if (!existing)
        return c.json({ ok: false, error: '존재하지 않는 제안건입니다' }, 404);
...
app.get('/:id/keyword-mappings', async (c) => {
    ...
    if (!id)
        return c.json({ ok: false, error: 'invalid id' }, 400);
    const rows = await query(`SELECT id, original_keyword, mapped_keyword
...
app.post('/:id/keyword-mappings', async (c) => {
    ...
    const originals = String(body.original_keyword ?? '')
        .split(',').map((s) => s.trim()).filter(Boolean);
    const mapped = String(body.mapped_keyword ?? '').trim();
    ...
    for (const orig of originals) {
        ...
        const existing = await query(`SELECT id FROM keyword_mappings WHERE project_id = $1 AND original_keyword = $2 LIMIT 1`, [projectId, orig]);
        if (existing[0]) {
            await query(`UPDATE keyword_mappings SET mapped_keyword = $1 WHERE id = $2`, [mapped, existing[0].id]);
            inserted.push({ id: existing[0].id, original_keyword: orig, mapped_keyword: mapped });
        }
        else {
            const row = await query(`INSERT INTO keyword_mappings (project_id, keyword_id, original_keyword, mapped_keyword)
         VALUES ($1,
           (SELECT id FROM keywords WHERE project_id = $1 AND keyword = $2 LIMIT 1),
           $2, $3)
         RETURNING id, original_keyword, mapped_keyword`, [projectId, orig, mapped]);
            if (row[0])
                inserted.push(row[0]);
        }
    }
...
app.delete('/:id/keyword-mappings/:mappingId', async (c) => {
    ...
    if (!projectId || !mappingId)
        return c.json({ ok: false, error: 'invalid id' }, 400);
    await query('DELETE FROM keyword_mappings WHERE id = $1 AND project_id = $2', [mappingId, projectId]);
