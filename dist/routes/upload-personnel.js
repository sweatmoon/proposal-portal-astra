...
export async function syncProposalMemberProfile(client, pid, name, updates) {
    const matches = await client.query('SELECT id FROM personnel WHERE TRIM(name) = TRIM($1) ORDER BY id', [name]);
  ...
  `, [pid, name]);
    ...
    const fields = ['auditor_grade', 'auditor_cert_no', 'phone', 'education_hours']
...
app.post('/', async (c) => {
    ...
    try {
        ...
        if (!file)
            return c.json({ ok: false, error: 'file 필드가 없습니다' }, 400);
    ...
    try {
        const result = await transaction(async (client) => {
            ...
            let safeEmail = personnel.email || null;
            if (safeEmail) {
                const existing = await client.query(`SELECT id FROM personnel WHERE email = $1 AND name != $2 LIMIT 1`, [safeEmail, personnel.name]);
