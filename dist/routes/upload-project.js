...
app.post('/debug', async (c) => {
    ...
    try {
        ...
        if (!file)
            return c.json({ ok: false, error: 'file 필드 없음' }, 400);
    ...
    try {
      ...
      `, [
            ...
            if (keywords.length > 0) {
                ...
                await client.query(`INSERT INTO keywords (project_id, keyword, sort_order) VALUES ${ph} ON CONFLICT DO NOTHING`, vals);
            }
            // 4. keyword_mappings (keyword_id JOIN)
            for (const km of keyword_mappings) {
                const kwRow = await client.query('SELECT id FROM keywords WHERE project_id = $1 AND keyword = $2', [projectId, km.original_keyword]);
                await client.query(`
        ...
        `, [phaseId, projectId, a.person_name, a.member_type, a.pre_survey_md, a.audit_md, a.action_confirm_md]);
            ...
            for (const m of proposal_members) {
                ...
                const matched = await client.query(`SELECT id FROM personnel WHERE TRIM(name) = TRIM($1) LIMIT 1`, [m.person_name]);
