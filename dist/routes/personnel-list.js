...
export function summarizeComplianceHistory(person, history, now = new Date()) {
    const ym = (value) => {
        const m = String(value ?? '').trim().match(/^(\d{4})[.\-/\s년]+(\d{1,2})(?:$|[.\-/\s월])/);
        return m && +m[2] >= 1 && +m[2] <= 12 ? `${m[1]}.${m[2].padStart(2, '0')}` : null;
    };
    const first = history.map(h => ym(h.audit_yearmonth)).filter((s) => s !== null).sort()[0]
    ...
    const isDirector = (role) => String(role ?? '').replace(/\s+/g, '').toUpperCase()
...
app.get('/:id/education-profile', async (c) => {
    ...
    try {
        ...
        if (!member)
            return c.json({ ok: false, error: 'person_not_in_proposal' }, 404);
        const person = await queryOne('SELECT education_name, education_hours, education_org FROM personnel WHERE id = $1', [personnelId]);
        if (!person)
            return c.json({ ok: false, error: 'person_not_found' }, 404);
...
app.get('/:id/photo-image', async (c) => {
    ...
    if (!person)
        return c.json({ ok: false, error: 'personnel_not_found' });
    c.header('Cache-Control', 'no-store');
