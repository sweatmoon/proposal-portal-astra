...
app.get('/:id/client-logo', async (c) => {
  ...
  if (!Number.isSafeInteger(id) || id <= 0) return c.json({ ok: false, error: 'invalid_id' }, 400)
  c.header('Cache-Control', 'no-store')
  try {
    const project = await queryOne<{ client_org: string | null }>('SELECT client_org FROM audit_projects WHERE id = $1', [id])
    if (!project) return c.json({ ok: false, error: 'project_not_found' }, 404)
    const clientOrg = String(project.client_org ?? '').trim()
    if (!clientOrg) return c.json({ ok: false, error: 'client_org_missing' }, 422)
