...
app.get('/proposals', async (c) => {
  ...
  let idx = 1
  if (status) { whereSql += ` AND p.proposal_status = $${idx++}`; params.push(status) }
  if (search) { whereSql += ` AND (p.project_name ILIKE $${idx} OR p.client_org ILIKE $${idx})`; params.push(`%${search}%`); idx++ }

  const countSql = `SELECT COUNT(DISTINCT p.id) AS total ${whereSql}`
  const countRow = await query<{ total: string }>(countSql, params)
...
app.get('/proposals/:id', async (c) => {
  ...
  const [phases, members, keywords, files, toc, kwMappings] = await Promise.all([
    ...
    query<Record<string, unknown>>(`SELECT * FROM keywords WHERE project_id = $1 ORDER BY sort_order, id`, [id]),
  ...
  const kwTags = keywords.map(k =>
    `<span class="inline-block px-2 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-full text-xs">${String(k.keyword ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;')}</span>`
  ).join(' ')
  ...
  const fmtRequirement = (value: unknown, unit: string): string => {
    if (value == null || String(value).trim() === '') return '미입력'
  ...
  <div id="autoModal" style="display:none">
    <div class="proposal-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="proposal-modal-heading">
      ...
      <details id="proposal-compliance-notes" style="font-size:13px;margin-bottom:12px;padding:12px;background:#f7f8fa;border-radius:8px">
        <summary>3.6 추가 제안 문구 (선택 입력)</summary>
        <p style="font-size:12px;color:#666">총괄 실적 수치는 수동 입력하지 않습니다. 아래 문구만 이번 생성에 반영하며 DB에는 저장하지 않습니다.</p>
        <label for="proposal-compliance-auditor-notes">감리원팀의 확인된 사업 경험 문구</label>
        <textarea id="proposal-compliance-auditor-notes" rows="2" style="display:block;width:100%;box-sizing:border-box;margin:4px 0 10px;padding:7px" placeholder="비워 두면 사업별 담당 분야에 맞춰 감리원 배치 문구를 사용합니다."></textarea>
        <label for="proposal-compliance-education">확정된 교육계획 문구</label>
        <textarea id="proposal-compliance-education" rows="2" style="display:block;width:100%;box-sizing:border-box;margin-top:4px;padding:7px" placeholder="비워 두면 임의의 교육 횟수나 계획을 작성하지 않습니다."></textarea>
      </details>
