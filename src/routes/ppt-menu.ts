...
const masterCategory = (raw?: string) => raw === undefined ? 'proposal' : (['proposal','attachment'].includes(raw) ? raw : null)
function masterResponse(row: Record<string, unknown>) {
  const metadata = row.layouts as { scope?: string; names?: string[] } | string[]
...
app.get('/master-templates', async c => {
  ...
  if (!category) return c.json({ ok:false, error:'잘못된 마스터 구분입니다' },400)
  ...
  } catch { return c.json({ok:false,error:'마스터 목록을 조회하지 못했습니다'},503) }
})
app.get('/master-templates/active', async c => {
  ...
  } catch { return c.json({ok:false,error:'활성 마스터를 조회하지 못했습니다'},503) }
})
app.post('/master-templates', async c => {
  ...
  try {
    ...
    if(!file || typeof file==='string' || !name || !file.name.toLowerCase().endsWith('.pptx')) return c.json({ok:false,error:'이름과 PPTX 파일이 필요합니다'},400)
    if(!file.size || file.size>20*1024*1024) return c.json({ok:false,error:'마스터 파일은 0바이트 초과 20MB 이하여야 합니다'},400)
    ...
    if(!layouts.length) return c.json({ok:false,error:'유효한 마스터 레이아웃이 없습니다'},400)
    ...
    const id=await transaction(async client=>{
      ...
      if(active) await client.query(`UPDATE ppt_master_templates SET is_active=0,updated_at=NOW() WHERE ${MASTER_SCOPE}=$1`,[category])
      const result=await client.query<{id:number}>(`INSERT INTO ppt_master_templates(name,description,pptx_b64,layouts,is_active) VALUES($1,$2,$3,$4,$5) RETURNING id`,[name,description,buffer.toString('base64'),JSON.stringify(metadata),active?1:0])
      return result.rows[0].id
    })
    return c.json({ok:true,id,layouts,category})
  } catch(e) { return c.json({ok:false,error:e instanceof Error?e.message:'마스터 등록 실패'},400) }
})
app.put('/master-templates/:id/activate',async c=>{
  const category=masterCategory(c.req.query('category')), id=Number(c.req.param('id'))
  if(!category || !Number.isSafeInteger(id) || id<=0) return c.json({ok:false,error:'잘못된 마스터 요청입니다'},400)
  try {
  ...
  } catch { return c.json({ok:false,error:'마스터 활성화 실패'},500) }
})
app.delete('/master-templates/:id',async c=>{
  ...
  try {
    ...
    return found?c.json({ok:true}):c.json({ok:false,error:'해당 구분의 마스터가 없습니다'},404)
  } catch { return c.json({ok:false,error:'마스터 삭제 실패'},500) }
})
